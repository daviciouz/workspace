/*
Below contains the code to generate every possible topsort combination of a given N node graph (O(n+m+c)). This code generates all topological sorts in the most efficient manner possible and can be outlined as follows:
1) Removing a node from a bag. O(1).
2) Reducing the predecessor count for the successors of the node removed from bag. O(k) where k is the number of successors of the node that was removed from the bag.
3) Initializing bag. O(n) where n is the number of nodes that have no predecessors.
4) Initializing predecessor count array. O(n) where n is the number of nodes
5) Initializing successor linked list. O(n) where n is the number of nodes.
NOTE: The following below contains two classes (Assignment1 and SuccLinkedList). You need both classes within your package to run the code.
The public static void main(String[] args) is in the Assignment1 class. */
