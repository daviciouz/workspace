...
public class programOne {
	...
	public static void topsorts()
	{
		if(bag.size > 0) {									  // if (bag is not empty)
			...
			{
				current = current.next;						  // pick up next element in the bag and put it in output array
				bag.remove(current);							
				...
				bag.removeAllFrom(last, size);				// remove all elements or numbers from the bag, thus empty the bag
			}
		} else {
